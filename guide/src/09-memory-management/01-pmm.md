...
being memory managment. Due to this subsystem being larger than previous
...
The first thing we need to know is how much RAM the computer has. There
is no hardware in the CPU that can tell us this, but the bios knows this
...
We will not be using the memory map for long, it is just a piece of
information that will allow us to initialize the PMM properly.

Second, we need to split memeory up into 4096 byte long frames, we do
...
bits where every bit maps to a a frame, and every zero means free and
...
the bootloader has known boot boot locations. and then everything else
...
init function is long and complicated becauase it involves thinking
...
in order to pass the memory map to C. Here is our code to do so:
...
entries that are recieved each interrupt call and pastes them to memory.

At the start before everything we set ds to 0, this is becuase we want
...
which entry of the memory map we want to recieve. bp is similar and will
...
isn\'t we go to the next entry. This is becuause int 15h sets ebx to 0
...
Now let\'s take a look at our header file:
...
We define the indended frame size and the memory address for the bitmap
...
printing the bitmap when testing. We then have our functions to be used
externally as we discussed before.
...
`init_pmm` full func and then go into explaining it now:
...
First of all we retrieve the memory map and the number of entries from
memory, and then we print out the memory map for debug purposes. We then
define the max usable address, after this we iterate throught the memory
map checking if the entry is type 1 (which means free). If it is free we
...
always round up in order to have the number of 32bit entires in the
...
These functions just use bit logic to set annd clear bits in the bitmap.
...
need. Next we need to edit our linker file in order to get the start and
...
code that i call at the end in order to visualise our bitmap and make
sure it is somewhat correct:
...
The freeing function just takes in the phycial address of the frame and
converts it to the index in the bitmap and then clears it.\
The allocating function iterates throught the bitmap until a free frame
is found, and if it is we set it and then return the physicall address.
Very simple! And that\'s the PMM all done, now we can move onto the next
stage of memory managment.
