...
The reason we need the run time stack is to enforce the lifecycle of
functions, so what happens when we call and return from a function,
etc.? We know that a program consists of many subroutines
(functions), and all these subroutines fill out a specific goal
within the program. Let's say a function called *y* starts its life
...
In x86, each process will have its own run-time stack; this is a
...
larger run-time stack, then removing it from memory.

The register `EBP` (stack frame base pointer) contains the
starting memory address of the current stack frame, the register
`ESP` contains the memory address of the top of the stack. To
push a new item to the stack, the `push` instruction can be used,
where the operand is the new item. It will decrement the value of ESP to
...
take up unnecessary resources when we can just change the existing data
when overwriting in the future.

#### The `cdecl` calling convention

When a function needs to call another, the caller should push all the
parameters that should be passed to be into the callee to the stack.
...
(stack pointer) to EBP (starting address of the current stack frame).
...
bytes, as we are in 32-bit protected mode. To reach a parameter using
...
first parameter. Remember the stack grows downwards, and the data
...
stack is the memory address we need to return to, This can just be
loaded into the program counter; this can be done using the x86
instruction called `RET` (which will also pop the value so we
will not need to pop it ourselves.)

Then, when our caller gets control again, we can pop our previous
parameters, just so we can clear up some memory space for the rest
...
as `cdecl`, or the C declaration. Many other conventions exist, and
...
visualizations where the stack grows upward that are used in other
systems. The reason why stacks grow downwards with x86, which is
...
when its value is 0, the stack will grow downwards, and to grow
upwards, its value should be 1.
