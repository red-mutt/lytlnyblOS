...
memory managment. These next 3 parts are going to be formatted in the
same way but instead of making memory managment we are making the
process managment subsystem. Currently our kernel just makes the cpu
execute one instruction after another from one stack, there is no such
thing as multitasking and running mutltiple things at the same time,
...
in order to continue executing some code later. For instance a process
must remember:
...
Tasking is simply just a loop that goas like:\
` Run A -> Save A -> Load B -> Run B -> Save B -> ...`

The process managment subsystem is split into these 3 parts:

1.  Process Managment: Responsible for creating and storing processes.
...
As we said before, a process is an **instance**, it is not the program
itself, it is just a saved execution state that contains the data we
...
2.  CPU state: The CPU has it\'s regsters like:
...
3.  Page Directory: A process must have a pointer to it\'s page
    directory, when the process runs, the page directory must be loaded
    into CR3.
4.  Stack: Each process needs a stack, so it would have it\'s own stack
    pointer that we store.
5.  State: This contians what is happing with the process, for example,
    the state can say that it is running etc\...
...
exists, just like before with the heap, we use a linked list however,
you have freedom here, you can use an array if you want, hoever you have
...
then need to free it\'s memory by freeing it\'s stack and destroying the
page directory for user processes. It\'s an important distinction that
we are currently **ONLY** making kernel procesesses, these would use the
...
in OS design. In linux this refers to an idle/swapper process, for now
...
-   PROCESS_BLOCKED: The process cannot currently continue because
    because it is waiting for some event or resources
-   PROCESS_SLEEPING: Similar to blocked, but the reason is primarly
    becuase of time
-   PROCESS_TERMINATED: The process has finished execution and should no
    longer be scheduled

For the registers datastructure it\'s alot similar to the one we had
with our interrupts, but instead we removed things that are useless like
the error code and interrupt number.

for the datastructure for the processes, we simple have the PID, the
...
Our functions then are pretty simple, we have initialisation, creation,
destruction and searching.

The last thing i haven\'t mentioned is the kernel_stack_bottom as an
external variable, for this we need look all the way back when we moved
...
First of all we initialise the process_head to NULL and the next_pid to
1.\
The next steps is building our main kernel_process sufficiently. So we
set the PID to 1, set the process to running, and use
`get_current_directory()` (a new function we will make) to get the page
directory that is currently being used (which is the kernel directory).

Next all our registers are set to 0, this is becuase the CPU state will
...
kmalloc in order to allocate some memory for the process and store it in
the heap. Next we then store the PID and state accodingly.
...
GDT. We then give a sensible eflags value and and set the page directory
...
Here\'s some simple code that we can put at the end of main in order to
