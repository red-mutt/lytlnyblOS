...
what we are making is simply used in order to switch the process
datastructures that we have made.
...
the code currently being executed is switching the process. So we
...
process data into the registers_t structure, this is becuase when the
IRQ wants to return after the timer interupt is done, it pops all the
...
advantage in order to load procecess, as well as save them.

## Simple test

I\'ll just show you the whole of my main in order to show you how simple
of a test this is.
...
exists and is used is becuase the code for printing that we are back in
...
switch. So we just wait a little before switching. If everything is good
