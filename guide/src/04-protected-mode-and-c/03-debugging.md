...
with real mode by the way). so `x/20i $pc` (which shows the next 20
...
then `source .gdbinit` in order to load our gdb init file, we should
then do `target remote localhost:1234 ` in order to connect to qemu,
eveything is set up now. set a breakpoint at `start` and type `c ` to
...
towards our jump, and you\'ll see it will jump to a completely
...
### Dissasemble instructions/functions
...
program counter, second does at an address, third does it at a a label
...
memory is being represented how we want it to. First does bytes, second
...
segment is set to `0x9000` at the `start` label. So we much change our
instruction to:
...
We should now be in real mode! yay!!! another good debugging technique
...
\"How could i even possibly realise that\" Then welcome to bare metal
programming :)
