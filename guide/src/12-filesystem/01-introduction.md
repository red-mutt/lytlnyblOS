...
`[bootloader][kernel][kernel][kernel]...` The filesystem we make is
...
will be similar to filesystems such as: `ext2`, `ext3` and `ext4`.
...
is metadata about the whole filesystem, ours may contain the:
...
for new types of storage devices, it will be easy to do so due to 
 the filesystem being abstracted from the driver.
...
another and can become confusing to navigate. Due to this, I will make a filesystem manager
...
user space programs, but I'm going to be embedding them into the filesystem manager, so our
filesystem becomes easier to use earlier on.
