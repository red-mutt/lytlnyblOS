...
that the syscalls require into `eax, ebx, ecx, edx`. Then we can access
this by checking our interrupts register type. We can use the same
...
terminated and perform a context switch to delete it, if we
...
It's obvious what this does, gets the PID for the currently running
...
Makes the process sleep for a certain amount of ticks, for this you
will have to store new data in the process data type, this is because
the syscall will just mark it as sleeping, set for how long and then
...
We can just set the stack as a universal address that starts out as
...
The first 2 things that we push are done to fill up the error code and
...
This code just traverses over it and check if the wake tick is greater than the
existing tick, if it's, we wake it up and set the process to ready.
...
terms of functionality. Basically we set the process to blocked and store
the buffer, size and count in the `reading_state`. And then basically,
like the sleeping syscall we then traverse though the list of processes
like in timer.c, but instead in keyboard.c:
...
have a loop that iterates until the end of the heap is 
covered by the amount of pages that are allocated; The contents of this
