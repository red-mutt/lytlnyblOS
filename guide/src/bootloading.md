...
After the block is provided, I will also be providing some comments on relatively ambiguous lines of code.
...
This is because, like in C, strings are treated as arrays, which is evident with the null pointer, which shows that we are at 
the end of 
...
Then we go into the `print_char` section; this will be called over and over until we print the end of our string
...
That wraps up our printing, which will be utilized a couple of times in the BIOS.
...
But I will explain it all as best as I can now in order of logical operation, and then in later chapters we can cover these x86concepts in the next chapter of this series.
...
well enough
What we do next is read the current cursor position; this is ultimately not needed, but we can do conditions for this to 
...
if it is not in the correct row.
...
But it's a bootloader that loads a kernel, which then outputs a message, which may seem pretty dull, but just consider the fact
